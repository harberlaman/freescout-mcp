...
var FreeScoutError = class extends Error {
  statusCode;
  endpoint;
...
// dist/config/loader.js
var MeilisearchConfigSchema = z.object({
  host: z.string().url(),
...
// dist/clients/freescout.js
var FreeScoutClient = class {
  baseUrl;
  apiKey;
  constructor(config) {
  ...
  async listConversations(params = {}) {
    ...
    const queryParams = {
      ...
      assignedTo: filterUnassigned ? void 0 : params.assignedTo,
    ...
    const response = await this.request(`/api/conversations${queryString}`);
    if (filterUnassigned) {
      const conversationsKey = Object.keys(response._embedded)[0];
      const allConversations = response._embedded[conversationsKey] || [];
      const unassignedConversations = allConversations.filter((conv) => !conv.assignee);
  ...
  async getConversation(id, embed = "threads") {
    const queryString = embed ? `?embed=${encodeURIComponent(embed)}` : "";
    return this.request(`/api/conversations/${id}${queryString}`);
  }
  ...
  async createThread(params) {
    ...
    if (bodyParams.user !== void 0)
      body.user = bodyParams.user;
    if (bodyParams.imported !== void 0)
      body.imported = bodyParams.imported;
    if (bodyParams.status)
      body.status = bodyParams.status;
    if (bodyParams.state)
      body.state = bodyParams.state;
    if (bodyParams.to && bodyParams.to.length > 0)
      body.to = bodyParams.to;
    if (bodyParams.cc && bodyParams.cc.length > 0)
      body.cc = bodyParams.cc;
    if (bodyParams.bcc && bodyParams.bcc.length > 0)
      body.bcc = bodyParams.bcc;
    if (bodyParams.createdAt)
      body.createdAt = bodyParams.createdAt;
    if (bodyParams.attachments && bodyParams.attachments.length > 0) {
      body.attachments = bodyParams.attachments;
    }
    const response = await fetch(`${this.baseUrl}/api/conversations/${conversationId}/threads`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      throw new FreeScoutError(errorMessage, response.status, `/api/conversations/${conversationId}/threads`);
    }
  ...
  async listTags(params = {}) {
    const queryString = this.buildQueryString(params);
    return this.request(`/api/tags${queryString}`);
  }
...
// dist/tools/list-conversations.js
var listConversationsSchema = z.object({
  ...
  status: z.string().optional().describe("Filter by status: active, pending, closed, spam. Comma-separated for multiple"),
  state: z.string().optional().describe("Filter by state: draft, published, deleted"),
  type: z.string().optional().describe("Filter by type: email, phone, chat"),
  assignedTo: z.number().optional().describe("Filter by assigned user ID. Use 0 for unassigned"),
  ...
  createdByUserId: z.number().optional().describe("Filter by the user ID who created the conversation"),
  createdByCustomerId: z.number().optional().describe("Filter by the customer ID who created the conversation"),
  createdSince: z.string().optional().describe("Filter to conversations created since this datetime (ISO 8601 UTC)"),
  updatedSince: z.string().optional().describe("Filter to conversations updated since this datetime (ISO 8601 UTC)"),
  sortField: z.string().optional().describe("Sort by field: createdAt, mailboxId, number, subject, updatedAt, waitingSince"),
  sortOrder: z.enum(["asc", "desc"]).optional().describe("Sort direction: asc or desc (default: desc)"),
  page: z.number().optional().describe("Page number (default: 1)"),
  pageSize: z.number().optional().describe("Results per page (default: 50)"),
  embed: z.string().optional().describe("Comma-separated related data to include: threads, timelogs, tags")
});
async function listConversations(client, input) {
...
// dist/tools/get-user.js
var getUserSchema = z.object({
...
// dist/tools/create-thread.js
var createThreadSchema = z.object({
  conversationId: z.number().describe("The conversation ID to add the thread to"),
  type: z.enum(["customer", "message", "note"]).describe("Thread type: customer (customer reply), message (agent reply), or note (agent note)"),
  text: z.string().describe("The message content"),
  customerId: z.number().optional().describe("Customer ID. Required when type is 'customer' (alternative to customerEmail)"),
  customerEmail: z.string().optional().describe("Customer email. Required when type is 'customer' (alternative to customerId)"),
  user: z.number().optional().describe("User ID of the agent. Required when type is 'message' or 'note'"),
  imported: z.boolean().optional().describe("When true, no outgoing emails or notifications are generated"),
  status: z.enum(["active", "pending", "closed"]).optional().describe("Change the conversation status"),
  state: z.enum(["draft", "published"]).optional().describe("Thread state (default: published)"),
  ...
  createdAt: z.string().optional().describe("Thread creation date (ISO 8601). Only valid when imported is true"),
  ...
  })).optional().describe("Array of attachments. Each must have fileName, mimeType, and either data or fileUrl")
}).refine((data) => {
  if (data.type === "customer") {
    return data.customerId !== void 0 || data.customerEmail !== void 0;
  }
  return true;
}, {
...
async function createThread(client, input) {
...
var searchSchema = z.object({
  ...
  status: z.array(z.number()).optional().describe("Filter by status code(s): 1=active, 2=pending, 3=closed"),
  mailboxId: z.array(z.number()).optional().describe("Filter by mailbox ID(s)"),
  userId: z.number().optional().describe("Filter by assigned user ID"),
  customerId: z.number().optional().describe("Filter by customer ID")
});
async function search(client, input) {
  const result = await client.search(input);
  return JSON.stringify(result, null, 2);
}

// dist/tools/list-tags.js
var listTagsSchema = z.object({
  conversationId: z.number().optional().describe("Filter tags by conversation ID. If not provided, returns all available tags"),
  page: z.number().optional().describe("Page number (default: 1)"),
  pageSize: z.number().optional().describe("Results per page (default: 50)")
});
async function listTags(client, input) {
  const result = await client.listTags(input);
  return JSON.stringify(result, null, 2);
}

// dist/tools/set-tags.js
var setTagsSchema = z.object({
  conversationId: z.number().describe("The conversation ID"),
  tags: z.array(z.string()).describe("Complete list of tag names to set on the conversation. WARNING: This REPLACES all existing tags. To add tags while preserving existing ones, first call list_tags to get current tags, then include all desired tags in this array.")
});
async function setTags(client, input) {
...
var toolDefinitions = [
  {
    ...
    description: "List conversations from FreeScout with filtering. Supports filtering by status, mailbox, tag, assignee, dates, and pagination.",
    inputSchema: listConversationsSchema
  },
  {
    name: "get_conversation",
    description: "Get a single conversation from FreeScout with full details including threads and tags (via embed option).",
    inputSchema: getConversationSchema
  },
  {
    name: "list_users",
    description: "List users from FreeScout with optional email filter and pagination.",
  ...
  {
    name: "create_thread",
    description: "Add a reply or note to a FreeScout conversation. Types: customer (customer reply, requires customerId or customerEmail), message (agent reply, requires user ID), note (internal note, requires user ID).",
  ...
  {
    name: "list_tags",
    description: "List available tags from FreeScout. Can optionally filter by conversation ID to get tags for a specific conversation.",
    inputSchema: listTagsSchema
  },
  {
    name: "set_tags",
    description: "Set tags on a FreeScout conversation. WARNING: This REPLACES all existing tags with the provided list. To add tags while preserving existing ones, first call list_tags to get current tags, then include all desired tags.",
...
async function handleToolCall(name, args, context) {
  switch (name) {
    case "list_conversations": {
      const input = listConversationsSchema.parse(args);
    ...
    case "search": {
      if (!context.meilisearchClient) {
        throw new Error("Meilisearch is not configured. Add meilisearch configuration to config.json to use search.");
      }
    ...
    case "set_tags": {
      const input = setTagsSchema.parse(args);
...
function addDescriptions(jsonSchema, zodSchema) {
